...
public class activationChamberRecipeCategory extends EOPRecipeCategory<BasicRecipe, activationChamberRecipeWrapper> {
	...
	public activationChamberRecipeCategory(IGuiHelper helper) {
		super("activation_chamber", "tile.activation_chamber.name",
				helper.createDrawable(background, xOffset, yOffset, 153, 65), BasicRecipe.class,
				new ItemStack(ModBlocks.activationChamber, 1));
		...
		fuel = helper.createAnimatedDrawable(helper.createDrawable(background, 178, 2, 3, 20), 200, StartDirection.TOP,
				true);
	}
	...
	@Nonnull
    @Override
	public IRecipeWrapper getRecipeWrapper(@Nonnull BasicRecipe recipe) {
		return new activationChamberRecipeWrapper(recipe);
	}

	@Override
	public void setRecipe(@Nonnull IRecipeLayout recipeLayout, @Nonnull activationChamberRecipeWrapper recipeWrapper,
						  @Nonnull IIngredients ingredients) {
		IGuiItemStackGroup guiItemStacks = recipeLayout.getItemStacks();
		guiItemStacks.init(0, true, 26 - xOffset - 1, 30 - yOffset - 1);
		guiItemStacks.init(1, false, 134 - xOffset - 1, 30 - yOffset - 1);
		guiItemStacks.init(2, true, 80 - xOffset - 1, 48 - yOffset - 1);
