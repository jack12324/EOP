...
public class DisablingPressRecipes extends EOPRecipes{
	//the list of activation results
	public static DisablingPressRecipes INSTANCE = new DisablingPressRecipes();
		private final Map<ItemStack[], ItemStack> activationList = Maps.<ItemStack[], ItemStack>newHashMap();
	 
	    
	    public DisablingPressRecipes()
	    {
				...
				for (int i = 0; i < stack.length; i++) {
					if (equal) {
						...
						for (int j = 0; j < stack.length; j++) {
							if ((stack2[j]!=null&&stack[i]!=null)&&stack2[j].getItem() == stack[i].getItem()&& (stack2[j].getMetadata() == 32767 || stack2[j].getMetadata() == stack[i].getMetadata())){
								equal = true;
								}
						}
					}
