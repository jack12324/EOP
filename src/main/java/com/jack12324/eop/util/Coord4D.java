...
/**
 * Coord4D - an integer-based way to keep track of and perform operations on
 * blocks in a Minecraft-based environment. This also takes in account the
 * dimension the coordinate is in.
 * 
 * @author aidancbrady
 *
 */
public class Coord4D {
	...
	/**
	 * Creates and returns a new Coord4D translated to the defined offsets of
	 * the side.
	 * 
	 * @param side
	 *            - side to translate this Coord4D to
	...
	/**
	 * Creates and returns a new Coord4D translated to the defined offsets of
	 * the side by the defined amount.
	 * 
	 * @param side
	 *            - side to translate this Coord4D to
	 * @param amount
	...
	public Coord4D offset(EnumFacing side, int amount) {
		...
		return new Coord4D(xCoord + (side.getFrontOffsetX() * amount), yCoord + (side.getFrontOffsetY() * amount),
	 ...
	 * @return EnumFacing representing the side the defined relative Coord4D is
	 *         on to this
	 */
	public EnumFacing sideDifference(Coord4D other) {
		...
		for (EnumFacing side : EnumFacing.VALUES) {
			if (side.getFrontOffsetX() == diff.xCoord && side.getFrontOffsetY() == diff.yCoord
					&& side.getFrontOffsetZ() == diff.zCoord) {
				return side;
			}
		}
	...
	/**
	 * Steps this Coord4D in the defined side's offset without creating a new
	 * value.
	 * 
	 * @param side
